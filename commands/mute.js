  ...
  if(message.member.roles.cache.some(role => ['Moderator'].includes(role.name))) {
    ...
    if(mention) {
      if(!mention.roles.cache.some(role => ["Moderator"].includes(role.name))) {
        ...
        color: 16733013,
        description: `That person is a moderator, therefore, I won't be muting them!`,
        author: {
          name: message.author.tag,
          icon_url: message.author.displayAvatarURL()
        }
      ...
      if(findname.first()) {
        ...
        message.channel.awaitMessages(filter, { max: 1, time: 60000 }).then(async collected => {
          if (collected.size === 0) {
            ...
            if(answer === "yes") {
              if(!findname.first().roles.cache.some(role => ["Moderator"].includes(role.name))) {
                ...
                color: 16733013,
                description: `That person is also a Moderator! Therefore, I won't be muting them. !`,
                author: {
                  name: message.author.tag,
                  icon_url: message.author.displayAvatarURL()
                }
